...
describe("When user is logged in", async () => {
  ...
  describe("And using valid inputs", async () => {
    ...
    test("should take use to the review screen after submitting", async () => {
      const text = await page.getContentsOf("h5");

      expect(text).toBe("Please confirm your entries");
    });

    test("should add blogs to index page after submitting", async () => {
      ...
      expect(title).toBe("My Testing Title");
...
describe("When user is not logged in", async () => {
  ...
  test("should prohibit any actions to the api", async () => {
